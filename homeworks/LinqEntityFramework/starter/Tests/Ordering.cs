...
public class OrderingTests //(DataContextFixture fixture) : IClassFixture<DataContextFixture>, IAsyncDisposable
{
    //private readonly ApplicationDataContext context = fixture.Context;

    [Fact]
    public async Task OrderValueTrend()
    {
        var ordering = new Ordering();//(context);
        var result = await ordering.OrderValueTrend();
    ...
    {
        var ordering = new Ordering();//(context);
        var result = await ordering.TopCustomers();
